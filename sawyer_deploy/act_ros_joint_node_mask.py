...
class ACTNode:
    def __init__(self):
        ...
        # --- HSV MASKING PARAMS (Matches Training Data exactly) ---
        self.lower_red1 = np.array([0, 100, 70])
        self.upper_red1 = np.array([10, 255, 255])
        self.lower_red2 = np.array([160, 100, 70])
    ...
    def img_callback(self, msg):
        try:
            ...
            # 2. Convert to HSV and create the combined Red/Pink mask
            ...
            # 3. Apply the mask (Blacks out the background)
            cv_img = cv2.bitwise_and(cv_img, cv_img, mask=mask)
            
            # --- Optional Debug Window (Shows live masked view) ---
            cv2.imshow("Live Masked View for ACT", cv_img)
            cv2.waitKey(1)
            # ------------------------------------------------------
            
            # 4. Convert masked BGR to RGB and format for the Neural Network
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if self.curr_img is not None and self.curr_qpos is not None and self.raw_qpos is not None:
                ...
                # print("Image and joint data received") # Commented out to prevent terminal spam

...
from thousand_tasks.training.act_bn_reaching.action_utils import action_to_pose, pose_to_action, \
    action_unscaling, action_scaling
# from thousand_tasks.training.utils import printarr
...
class Evaluator:
    ...
    def init_trajectory(self, task_name):
        ...
        self.action_history = np.array([[0., 0., 0., 0., 0., 0., self.not_terminate_action]] * self.history_size)

        self.eef_pose = self.neutral_pose.copy()
    ...
    def step(self, policy):
        ...
        # Get current state.
        curr_state = pose_to_action(T_WE, 0, self.done)
    ...
    def calculate_errors(self, task_name):
        bottleneck_pose = self.bottleneck_poses[task_name][self.current_task_idx].copy()
        delta_pos = np.linalg.norm(bottleneck_pose[:3, 3] - self.eef_pose[:3, 3])
        delta_rot = np.rad2deg(np.linalg.norm(rot2rotvec(bottleneck_pose[:3, :3] @ self.eef_pose[:3, :3].T)))
    ...
    def evaluate_policy(self, policy, num_rollouts_per_task=10, return_individual_results=False):
        ...
        for task_name in self.task_names:
            ...
            for _ in range(num_rollouts_per_task):
                ...
                delta_pos, delta_rot = self.calculate_errors(task_name)

                pos_errors[task_name].append(delta_pos * 1e3)
                rot_errors[task_name].append(delta_rot)
...
if __name__ == '__main__':
    ...
    #     policy_path = f'/home/kamil/phd/head_cam/thousand_tasks/baselines/mt_act/bn_reaching/runs/{args.name}/{args.name}.pt'
    # elif pc_name == 'tiger' or pc_name == 'omen':
    #     dset_root = join('/home/kamil/phd/osil/assets', args.dir)
    #     policy_path = f'/baselines/bn_reaching/runs/{args.name}/{args.name}.pt'
    # else:
    #     dset_root = join('/mnt/data/kamil', args.dir)
    #     policy_path = f'/baselines/bn_reaching/runs/{args.name}/{args.name}.pt'

    dset_root = '/home/vv19/Downloads/test_osil'
    policy_path = '/home/vv19/projects/python/osil/thousand_tasks/baselines/act_bn_reaching/runs/test_delta/final.pt'
    ...
    print(generator.evaluate_policy(policy, num_rollouts_per_task=1, return_individual_results=False))

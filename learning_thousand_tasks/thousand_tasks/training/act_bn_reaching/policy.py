...
from thousand_tasks.training.act_bn_reaching.config import mt_act_args, mt_act_args_dict
...
class ACTPolicy(nn.Module):
    def __init__(self, args_override):
        ...
        self.register_buffer('trans_rot_mean', torch.tensor(np.zeros(6), dtype=torch.float32))
        self.register_buffer('trans_rot_std', torch.tensor(np.ones(6), dtype=torch.float32))
    ...
    def __call__(self, data):
        ...
        if len(actions.shape) > 1:  # training time
            ...
            #                                                                    actions[:, :, -2],
            #                                                                    reduction='none')  # TODO use pos_weight to counter class imbalance
            #
            termination_bce = torch.nn.functional.binary_cross_entropy_with_logits(a_hat[:, :, 6],
                                                                                   actions[:, :, 6],

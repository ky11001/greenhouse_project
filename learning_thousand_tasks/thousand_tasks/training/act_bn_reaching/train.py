...
from thousand_tasks.training.act_bn_reaching.policy import ACTPolicy
from thousand_tasks.training.act_bn_reaching.evaluator import Evaluator
...
if __name__ == '__main__':
    # --- Arg parser ---
    parser = argparse.ArgumentParser(description="BC Bottleneck Reaching (Alignment) Training Script")
    parser.add_argument('-name', '--name', type=str, required=False,
                        help='Name for the training run (used for checkpoint naming)', default='bc_alignment')
    ...
    parser.add_argument('-act_mode', '--action_mode', type=str, required=False,
                        help="Action mode: 'delta' or 'abs_delta' (overrides config)", default=None)
    ...
    if args.hidden_dimension is not None:
        ...
        mt_act_args_dict['global_nn_dims_2'][-1] = args.hidden_dimension
    if args.action_mode is not None:
        mt_act_args_dict['action_mode'] = args.action_mode

    folder_name = 'bn_reaching_processed'
    ...
    # The sampler dynamically loads chunks into memory during training
    sampler = DistributedCustomSampler(
        dataset=dset_train,
        batch_size=batch_size,
        chuncks2load=1,  # Load 1 chunk at a time (500 datapoints per chunk)
        use_distributed=False,  # Set to True if using multiple GPUs
        verbose=False,
        seed=0
    )
    ...
    policy.trans_rot_mean = dset_train.trans_rot_mean
    policy.trans_rot_std = dset_train.trans_rot_std
    ...
    policy.num_dont_term_acts = torch.tensor(dset_train.num_dont_term_acts)
    policy.to(args.device)

    # load weights
    # policy.load_state_dict(torch.load(f'/home/kamil/phd/head_cam/thousand_tasks/baselines/mt_act/runs/{args.name}/{args.name}.pt'))
    # eval_policy(policy, data_loader_train)
    # raise
    ...
    # dset = BCDataset(root, config=mt_act_args_dict, reprocess=False, visualise=True)
    # data_loader_viz = DataLoader(dset, batch_size=1, shuffle=False, num_workers=0)
    # eval_policy(policy, data_loader_viz)

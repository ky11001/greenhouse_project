...
mt_act_args_dict = {
    ...
    'preprocessing_num_clusters_total': 25,
    'preprocessing_num_clusters_to_mask': 5,
    'preprocessing_pcd_noise_std': 1.5e-3,
    'preprocessing_act_hist_noise_std': 0.2,  # 0.1 looks ok when visualising
    'trans_spacing_between_waypoints': 1e-2,

    'trans_perturb_to_bn_lb': np.array([-0.15, -0.15, -0.05]),
    'trans_perturb_to_bn_ub': np.array([0.15, 0.15, 0.05]),
    'rot_perturb_to_bn_lb': np.array([-np.deg2rad(-0), -np.deg2rad(-0), -np.deg2rad(10)]),
    'rot_perturb_to_bn_ub': np.array([np.deg2rad(0), np.deg2rad(0), np.deg2rad(10)]),

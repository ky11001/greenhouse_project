...
class Evaluator:
    ...
    def __init__(self, root, config, which_camera: str = 'head'):
        ...
        for task_folder_name in self.task_dirs:
            ...
            self.bottleneck_poses[task_name].append(posevec2pose(np.load(join(task_dir, 'demo_eef_posevecs.npy'))[0][:7]))
    ...
    def step(self, policy):
        ...
        if self.num_clusters_to_mask > 0:
...
def get_trajectory_error(pred_traj: np.ndarray, gt_traj: np.ndarray):
    # Find index of point in rollout closest to bottleneck pose and assume that the interaction starts from there
    start_idx = np.argmin(np.linalg.norm(pred_traj[:, :3, 3] - gt_traj[0, :3, 3], axis=1))

    trans_error_1, rot_error_1 = get_single_trajectory_error(pred_traj[start_idx:], gt_traj)
    trans_error_2, rot_error_2 = get_single_trajectory_error(gt_traj, pred_traj[start_idx:])
